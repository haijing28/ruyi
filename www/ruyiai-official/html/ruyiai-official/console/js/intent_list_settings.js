...
function sysIntentListPage($rootScope,$scope, $state, $stateParams){
    // 场景开关
    ...
    $("body").off("click",".new-user-next").on("click",".new-user-next",function($event){
        ...
        switch(true){
            ...
            case guideId == 4:
                ...
                break;
            case guideId == 5:
                $(".action-tips p").html('点这里，试试你刚才写的问答！<br>更多新手教程，<a href="https://docs.ruyi.ai/41_xin_shou_ru_men.html" target="_black" style="text-decoration: underline;">点击这里</a>' );
                $(".action-tips").css({
                    "left":"2%",
            ...
            $.confirm_save({
                ...
                "ensureFn": function() {
                    ...
                    if(!$("[data-act=confirmShow]").css("display")){
                        if($scope.scenario.scenarioType == "SYSTEMACTION"){
                            window.location.href = "#/intent_list/" +scenarioId+ "/intent_event_detail/" + intent_id;
                        }else{
...
//              data:{"start":0, "limit":$scope.pageSize, "keywords": searchIntentWords, "sortkey": sortkey},
//              method: "GET",
//              success: function(data){
//                  data = dataParse(data);
//					if(data.code == 0){
